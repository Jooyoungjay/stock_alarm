...
test('parseRoadmapMarkdown extracts roadmap metadata and next task', async () => {
  ...
  assert.equal(roadmap.recommendedOrder[0], '가격 차이 이상치 모니터링');
  assert.equal(roadmap.nextTask.title, '가격 차이 이상치 모니터링');
...
test('parseRoadmapMarkdown normalizes explicit WBS task statuses', async () => {
  ...
  const pendingKisDriftTask = roadmap.sections
    .find((section) => section.id === '6')
  ...
  assert.equal(pendingKisDriftTask.status, 'pending');
