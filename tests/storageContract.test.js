...
test('createStore returns a contract-compliant JsonStore', async () => {
...
test('storage engine normalization accepts json and rejects unsupported values', () => {
  assert.equal(normalizeStorageEngine(' JSON '), STORAGE_ENGINES.JSON);
  assert.equal(normalizeStorageEngine(''), STORAGE_ENGINES.JSON);
  assert.throws(() => normalizeStorageEngine('postgres'), /지원하지 않는 저장소 엔진/);
