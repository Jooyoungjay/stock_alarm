...
- `check:observation --save-history`로 저장된 점검 히스토리, 최근 READY/NOT READY, 직전 대비 변화 확인
- 배당 새로고침
- 데이터 백업 생성
- 자동 백업 즉시 실행
- 백업 목록 확인
...
- 보호 대상 운영 API는 `/api/health`, `/api/data-model`, `/api/roadmap`, `/api/observation-issues`, `/api/observation-history`, `/api/backups`, `/api/check-now`, `/api/dividends/refresh`, `/api/dividend-alerts/check`, `/api/briefing/send`, `/api/telegram/test`입니다.
...
- 관리자 화면에는 서버 상태, 데이터 모델, provider 진단, 백업/복구/삭제, 개발 WBS, 실사용 OBS 이슈, 점검 히스토리를 옮겼습니다.
