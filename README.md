...
- WBS 14.6 배당 실패 다음 조치 통일 — [개발 WBS](docs/development-roadmap.md) §14
...
JSON 저장소의 `devices`·`deviceId` 필드와 알림 기록의 `pushDelivery*` 필드는 기존 데이터 호환을 위해 남아 있습니다. 신규 알림은 텔레그램만 전송합니다.
...
1. ~~WBS·README·AGENTS 정합성~~ (14.1 완료)
2. ~~AI 팀 점진 개선 스킬~~ (14.2 완료)
3. ~~실사용 백로그 triage~~ (14.3 완료)
4. ~~주간 회귀 운영 루틴~~ (14.4 완료)
5. ~~텔레그램 `/brief` 가독성~~ (14.5 완료)
6. 배당 실패 다음 조치 통일 (14.6 — **다음**)
7. KIS/Naver 자동 비교 알림 노이즈 (14.7)
8. JSON 레거시 필드 정리 계획 (14.8)

WBS 13 (개인 로컬 운영 전환)은 13.1~13.10까지 완료했습니다. WBS 14는 [개발 WBS 및 로드맵](docs/development-roadmap.md) §14와 [AGENTS.md](AGENTS.md) §현재 우선순위를 단일 기준으로 봅니다.
