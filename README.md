...
- 관리자 화면에는 서버 상태, 데이터 모델, provider 진단, 백업/복구/삭제, 개발 WBS, 실사용 OBS 이슈, 점검 실행/저장, 점검 히스토리 상세/다운로드/삭제/정리를 옮겼습니다.
