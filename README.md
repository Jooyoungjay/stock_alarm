...
- 관리자 KIS/Naver 가격 비교: 같은 종목의 Naver 기준가와 KIS KRX/NXT/통합 가격 차이 확인, 추천 시장 적용, 이상치 기준 초과 시 주의/경고 표시, 최근 비교 이력 저장, 시장별 추세와 추세 기반 적용 판단 표시, 관심 종목 자동 점검과 결과 알림
...
- KIS/Naver 가격 비교: 같은 종목의 Naver 기준가와 KIS 시장별 가격 차이, 차이율, 실패 사유 확인, 추천 시장 적용, 이상치 기준 초과 여부 확인, 최근 비교 이력, 시장별 추세, 추세 기반 적용 판단, 관심 종목 자동 점검 결과와 알림 상태 확인
- 배당 provider 진단 확인
- 개발 로드맵과 다음 개발 순서 확인
- 배당 새로고침
- 데이터 백업 생성
...
- 가격 비교 이슈 처리 UX
- 자동 KIS/Naver 비교 알림에서 나온 이슈를 관리자 화면에서 확인, 보류, 해결 처리할 수 있게 정리
...
- 증권사 API: 한국투자증권/키움 quote-only adapter 점검 CLI, 주문 기능 차단 가드, KIS 현재가 provider, KIS 토큰 자동 발급/캐시, 실계정 현재가 smoke test CLI, 관리자 KIS 현재가 점검, 종목별 KIS 시장 기준, KIS/Naver 가격 비교와 시장 적용, 가격 차이 이상치 모니터링, 가격 비교 이력 저장, 시장별 가격 비교 추세, 추세 기반 시장 추천, 관심 종목 자동 점검, 자동 점검 결과 알림, 환경변수 문서화
