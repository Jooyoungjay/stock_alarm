...
- 관리자 KIS/Naver 가격 비교: 같은 종목의 Naver 기준가와 KIS KRX/NXT/통합 가격 차이 확인, 추천 시장을 등록 종목에 바로 적용
...
- KIS/Naver 가격 비교: 같은 종목의 Naver 기준가와 KIS 시장별 가격 차이, 차이율, 실패 사유 확인, 추천 시장을 종목 기준으로 바로 적용
- 배당 provider 진단 확인
- 개발 로드맵과 다음 개발 순서 확인
- 배당 새로고침
- 데이터 백업 생성
...
`/admin`의 `KIS/Naver 가격 비교` 카드는 같은 종목의 Naver 기준가와 KIS KRX/NXT/통합 가격을 나란히 조회합니다. 시장별 가격 차이와 차이율, provider 실패 사유를 볼 수 있고, 가격 차이가 가장 작은 추천 시장을 등록 종목의 KIS 시장 기준으로 바로 적용할 수 있습니다.
...
- 증권사 API: 한국투자증권/키움 quote-only adapter 점검 CLI, 주문 기능 차단 가드, KIS 현재가 provider, KIS 토큰 자동 발급/캐시, 실계정 현재가 smoke test CLI, 관리자 KIS 현재가 점검, 종목별 KIS 시장 기준, KIS/Naver 가격 비교와 시장 적용, 환경변수 문서화
