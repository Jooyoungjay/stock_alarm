...
`기준 미리 확인`을 누르면 현재가, 구매일 이후 최고가, 알림 기준가, 현재 하락률을 저장 전에 확인할 수 있습니다.
...
현재 배당주 전용 기능은 아직 구현하지 않았습니다. 앱 방향성에는 잘 맞으므로 아래 항목을 후보로 둡니다.

- 배당 수익률, 배당락일, 지급일 표시
- 배당락일 전후 알림
- 월/분기/연 배당 예상 현금흐름 보기
- 보유 수량 입력 후 예상 배당금 계산
- 배당 성장률과 최근 배당 변경 내역 표시

다만 이 기능은 안정적인 배당 데이터 소스가 필요합니다. 현재 무료 시세 provider만으로는 배당락일과 지급일을 안정적으로 보장하기 어렵기 때문에, 먼저 감시/알림 UX를 완성한 뒤 별도 데이터 소스를 정해 개발하는 것이 좋습니다.
