...
- 일반, ISA, 연금, 기타 계좌 구분 저장. 같은 종목도 계좌가 다르면 별도 보유분으로 관리
...
매수가와 보유 수량이 있는 종목 카드에는 `추가매수 계산기`가 표시됩니다. 같은 종목을 일반 계좌와 ISA 계좌에 모두 등록한 경우, 계산기는 해당 종목 카드의 계좌 보유분만 대상으로 합니다.
...
1. 종목 검색 또는 종목 코드 입력, 필요하면 계좌 구분, KIS 시장 기준, 종목 상태 선택
...
| `표시 이름` | 예: `두산퓨얼셀` |
| `계좌 구분` | 일반, ISA, 연금, 기타. 같은 종목도 계좌가 다르면 별도 보유분으로 등록 가능 |
...
- `CSV 내보내기`는 현재 등록된 종목의 종목코드, 계좌 구분, 상태, 매수가, 수량, 알림 기준, 배당 정보, 투자 계획 메모를 CSV로 저장합니다.
- `CSV 가져오기`는 파일 전체를 먼저 검증합니다. 종목코드 누락, 같은 계좌 안의 중복 종목, 잘못된 숫자/날짜, 알림 기준별 필수값 누락이 있으면 등록을 시작하지 않습니다.
...
/add symbol=336260 name=두산퓨얼셀 account=isa price=88779 market=NX type=profit rate=10
```
...
계좌 구분은 한 줄 등록에서는 기본값 `일반`으로 들어갑니다. ISA, 연금, 기타 계좌로 넣으려면 `symbol=... account=isa price=...` 형식의 키 입력을 사용하세요.
...
같은 종목이 여러 계좌에 있으면 `336260@general`, `336260@isa`, `336260@pension`처럼 계좌를 붙여 대상을 지정합니다.
...
- 기기별 푸시 토큰
- 등록 종목
- 계좌 구분, 매수가, 보유 수량, 주당 연 배당금, 배당 주기, 배당 지급월, 매수일
- 종목 상태. 보유, 관심, 매도
- 최고가
- 알림 기준
