...
Stock Alarm은 매수한 종목의 가격을 주기적으로 확인하고, 사용자가 정한 매도 기준에 도달하면 텔레그램으로 반복 알림을 보내는 개인용 로컬 MVP입니다.

현재 단계는 **개인 PC 로컬 웹앱 + 텔레그램 봇 중심 MVP**입니다. 2026-06-22 기준으로 Postgres 전환, 앱스토어/플레이스토어 출시, Railway 상시 배포는 진행하지 않습니다.

앞으로의 개발 방향:

- 개인 PC에서 `start-local.bat`, `status-local.bat`, `stop-local.bat`로 실행하고 관리
- 텔레그램 알림과 명령어로 가격, 배당, 브리핑, 상태 확인
- 데이터 저장은 로컬 JSON과 자동 백업 중심으로 유지
- Postgres, 앱 심사/스토어, Railway 관련 코드는 정리 완료 또는 `docs/archive/` 보관 (모바일 앱/API는 WBS 13.4에서 제거)

## 빠른 시작

AI 에이전트로 작업할 때는 [AGENTS.md](AGENTS.md)의 역할(@pm, @backend, @qa 등)과 WBS 완료 조건을 먼저 확인하세요.
...
아래 목록에는 현재 개인 로컬 운영에 계속 쓸 기능과 과거 방향에서 만든 정리 후보가 함께 포함되어 있습니다. 실제 운영 핵심은 웹 대시보드, 텔레그램, 로컬 JSON 저장소, 백업/복구, 시세/배당 진단입니다.
...
- 일반, ISA, 연금, 기타 계좌 구분과 증권사/계좌명 저장. 같은 종목도 `일반 · 키움`, `일반 · 미래에셋`, `ISA · NH`처럼 별도 보유분으로 관리
...
- 배당락일과 지급일 기준 텔레그램 알림. 모바일 푸시는 정리 후보
- 이익금 반납률 기준을 이해할 수 있도록 최고가 기준 최대 평가이익과 반납 금액 표시
- 포트폴리오 요약에서 계좌별 총 최대 평가이익, 총 반납 금액, 총 반납률 표시
...
- 저장된 로컬 점검 히스토리를 관리자 화면에서 최근 결과와 직전 대비 변화로 표시
- 관리자 화면에서 로컬 실사용 점검을 실행하고 결과를 히스토리에 바로 저장
- 관리자 화면에서 저장된 점검 히스토리의 항목별 evidence와 다음 조치를 상세 확인하고 JSON으로 다운로드
- 관리자 화면에서 점검 히스토리 파일을 개별 삭제하고 최근 N개만 남기도록 오래된 기록 정리
- 실패/수동 점검 항목별 조치 상태, 메모, 다음 확인일 저장
...
- 저장소 계약과 JsonStore 백업 스냅샷 export/import 검증
...
매수가와 보유 수량이 있는 종목 카드에는 `추가매수 계산기`가 표시됩니다. 같은 종목을 여러 증권사 일반 계좌 또는 ISA 계좌에 나눠 등록한 경우, 계산기는 해당 종목 카드의 계좌명 보유분만 대상으로 합니다.
...
아래 구조에는 현재 운영에 쓰는 파일과 정리 후보가 함께 들어 있습니다. 개인용 로컬 운영의 핵심은 `public/`, `src/`, `scripts/local-server.js`, `data/`, `start-local.bat`, `status-local.bat`, `stop-local.bat`입니다. 과거 Expo 모바일 앱은 WBS 13.4에서 제거했고 `docs/archive/`에 설정 스냅샷을 보관합니다. 앱 심사/스토어/Railway 문서는 후속 WBS에서 제거 또는 보관 위치 이동을 결정합니다.
...
│  ├─ sw.js
│  └─ icons/
├─ docs/archive/            # 제거된 Postgres·모바일 설계/설정 보관
│  ├─ json-to-db-migration.md
│  ├─ mobile-app.json
│  ├─ mobile-real-device-e2e.md
│  └─ store-listing.ko.json
├─ src/
│  ├─ server.js             # HTTP 서버와 API
│  ├─ alertEngine.js        # 알림 기준 계산, 상태 추적, 텔레그램 알림 전송
...
│  ├─ storageFactory.js     # JsonStore 생성
...
│  ├─ archive/                     # 제거된 Postgres·모바일·스토어·Railway 보관
│  │  ├─ json-to-db-migration.md
│  │  ├─ mobile-app.json
│  │  ├─ mobile-real-device-e2e.md
│  │  ├─ store-listing.ko.json
│  │  ├─ app-store-review-prep.md
│  │  ├─ https-demo-server.md
│  │  ├─ store-screenshots.md
│  │  ├─ store-submission-assets.md
│  │  ├─ privacy-policy-ko.md
│  │  ├─ railway-deploy.md
│  │  ├─ railway.json
│  │  └─ .env.railway.example
│  ├─ user-admin-page-split.md     # 사용자/관리자 화면 분리 전략
│  ├─ scripts-and-env-classification.md
│  ├─ visual-regression-check.md   # 브라우저 시각 회귀 점검 절차
│  ├─ full-regression-test-scenarios.md
│  └─ ...                   # 시세 API 검토, 관찰 리포트 등
├─ tests/                   # Node.js 테스트
│  ├─ helpers/storageSnapshotContract.js # 저장소별 백업 스냅샷 계약 검증 헬퍼
│  └─ fixtures/storage-snapshot/   # JsonStore 스냅샷 계약 검증용 fixture
...
**최소 필수:** `TELEGRAM_BOT_TOKEN`, `TELEGRAM_CHAT_ID`  
**script/env 분류표:** [scripts-and-env-classification.md](docs/scripts-and-env-classification.md) (WBS 13.2 — 일상 운영 / 품질 점검 / 제거 완료 항목 구분)
...
아래는 개인 PC 로컬 운영에 자주 쓰는 값입니다. 가격·배당 알림은 **텔레그램만** 사용합니다.
...
| `KIS_NAVER_AUTO_COMPARE_RESOLVED_REOPEN_COOLDOWN_MINUTES` | `1440` | 해결 처리한 이슈가 다시 감지됐을 때 텔레그램 재알림을 보내기 전 대기 시간 |
...
npm 스크립트 ([분류표](docs/scripts-and-env-classification.md)):
...
npm run check:publicdata-price
npm run kis:token
```
...
- 점검 실행/저장 버튼으로 로컬 실사용 점검을 실행하고 최근 READY/NOT READY, 직전 대비 변화 확인
- 배당 새로고침
- 데이터 백업 생성
- 자동 백업 즉시 실행
- 백업 목록 확인
...
- 보호 대상 운영 API는 `/api/health`, `/api/data-model`, `/api/roadmap`, `/api/observation-issues`, `/api/observation-history`, `/api/observation-history/{fileName}`, `/api/observation-history/{fileName}/results/{resultId}/action`, `/api/observation-history/run`, `/api/observation-history/prune`, `/api/backups`, `/api/check-now`, `/api/dividends/refresh`, `/api/dividend-alerts/check`, `/api/briefing/send`, `/api/telegram/test`입니다.
...
- 관리자 화면에는 서버 상태, 데이터 모델, provider 진단, 백업/복구/삭제, 개발 WBS, 실사용 OBS 이슈, 점검 실행/저장, 점검 히스토리 상세/다운로드/삭제/정리/조치 메모를 옮겼습니다.
...
1. 종목 검색 또는 종목 코드 입력, 필요하면 계좌 구분, 증권사/계좌명, KIS 시장 기준, 종목 상태 선택
...
| `표시 이름` | 예: `두산퓨얼셀` |
| `계좌 구분` | 일반, ISA, 연금, 기타. 세금/상품 유형을 구분 |
| `증권사/계좌명` | 키움 일반, 미래에셋 일반, NH ISA 등. 같은 종목도 계좌명이 다르면 별도 보유분으로 등록 가능 |
...
같은 카드의 `자동 점검 실행` 버튼은 등록된 활성 국내 종목을 대상으로 KIS/Naver 비교를 한 번에 실행하고 최근 이력과 추세 추천을 갱신합니다. 반복 자동 점검은 실제 API 호출이 발생하므로 기본값은 꺼짐이며, 필요할 때 `.env`에 `KIS_NAVER_AUTO_COMPARE_ENABLED=true`를 넣고 서버를 재시작해 켭니다. 기본 주기는 `KIS_NAVER_AUTO_COMPARE_INTERVAL_SECONDS=21600`(6시간), 한 번에 비교하는 종목 수는 `KIS_NAVER_AUTO_COMPARE_LIMIT=5`입니다. 자동 비교에서 실패, 반복 괴리, 추천 시장 변경, 추가 확인 판단이 나오면 텔레그램으로 요약 알림을 보냅니다. 같은 이슈는 종목·시장 기준 안정 키로 묶어 중복 전송하지 않고, `확인`·`보류` 처리한 이슈는 재알림하지 않으며, `해결` 후 재감지는 열림으로 되돌린 뒤 `KIS_NAVER_AUTO_COMPARE_RESOLVED_REOPEN_COOLDOWN_MINUTES`(기본 24시간) 이후에만 재알림합니다. 운영 절차는 [KIS/Naver 자동 비교 알림 운영](docs/personal-kis-naver-alert-operations.md)을 참고하세요. 관리자 API는 `POST /api/kis/naver-compare/auto-run`, `PATCH /api/kis/naver-compare/issues`이며 `ADMIN_TOKEN` 보호 대상입니다.
...
- `CSV 내보내기`는 현재 등록된 종목의 종목코드, 계좌 구분, 계좌명, 상태, 매수가, 수량, 알림 기준, 배당 정보, 투자 계획 메모를 CSV로 저장합니다.
- `CSV 가져오기`는 파일 전체를 먼저 검증합니다. 종목코드 누락, 같은 계좌 구분/계좌명 안의 중복 종목, 잘못된 숫자/날짜, 알림 기준별 필수값 누락이 있으면 등록을 시작하지 않습니다.
...
- WBS 19 **장중 오늘 할 일·알림 연동 심화** — **19.1~19.8 완료** ([로드맵](docs/development-roadmap.md) §19, [wbs-19-evolution 스킬](.cursor/skills/wbs-19-evolution/SKILL.md))
- WBS 20 **개인 운영 회귀·문서 정합 3차** — **20.1~20.8 완료** ([로드맵](docs/development-roadmap.md) §20, [wbs-20-evolution 스킬](.cursor/skills/wbs-20-evolution/SKILL.md))
- **운영 유지 모드** — WBS 13~20 완료. 신규 작업은 [백로그](docs/personal-backlog.md) triage만 사용
- WBS 18 **장중 알림·점검 피드백 루프** — **18.1~18.8 완료** ([wbs-18-evolution 스킬](.cursor/skills/wbs-18-evolution/SKILL.md))
- WBS 17 **장중·원격 운영 신뢰도 심화** — **17.1~17.8 완료** ([wbs-17-evolution 스킬](.cursor/skills/wbs-17-evolution/SKILL.md))
- WBS 16 **개인 운영 편의성 개선** — **16.1~16.9 완료** ([wbs-16-evolution 스킬](.cursor/skills/wbs-16-evolution/SKILL.md))
...
| `/brief` | 위험 종목·이익금 반납·배당·평가 요약 (일일 브리핑) |
...
/add symbol=336260 name=두산퓨얼셀 account=isa broker=키움 price=88779 market=NX type=profit rate=10
```
...
계좌 구분은 한 줄 등록에서는 기본값 `일반`으로 들어갑니다. ISA, 연금, 기타 계좌로 넣으려면 `symbol=... account=isa price=...` 형식의 키 입력을 사용하세요. 같은 일반 계좌가 여러 증권사에 있으면 `broker=키움`, `broker=미래에셋`처럼 계좌명을 같이 입력합니다.
...
같은 종목이 여러 계좌에 있으면 `336260@general`, `336260@isa`, `336260@pension`처럼 계좌 구분을 붙입니다. 같은 구분 안에 여러 계좌가 있으면 `336260@키움`, `336260@미래에셋`처럼 계좌명을 붙여 대상을 지정합니다.
...
- `name`: 표시 이름 변경
- `account`: 계좌 구분 변경. 값은 `general`, `isa`, `pension`, `other`
- `accountname`: 증권사/계좌명 변경. 예: `/edit 336260 accountname 키움 일반`
...
- 기기별 푸시 토큰
- 등록 종목
- 계좌 구분, 증권사/계좌명, 매수가, 보유 수량, 주당 연 배당금, 배당 주기, 배당 지급월, 매수일
- 종목 상태. 보유, 관심, 매도
- 최고가
- 알림 기준
...
- 과거 DB 이전 설계 기록은 [docs/archive/json-to-db-migration.md](docs/archive/json-to-db-migration.md)에 보관되어 있습니다.

저장소:

- 개인 로컬 운영은 **JsonStore**(`data/store.json`)만 사용합니다.
- 저장소 계약은 `src/storageContract.js`, 생성은 `src/storageFactory.js`가 담당합니다.
- `tests/storageSnapshotContract.test.js`가 JsonStore 백업 스냅샷 export/import round-trip을 검증합니다.
- 백업/복구는 저장소 스냅샷 export/import 계약을 사용합니다.
...
## 모바일 앱/API (제거 완료 — WBS 13.4)

> **보관** — `mobile/` Expo 앱, `/api/mobile/*`, `POST /api/devices` API는 제거했습니다. 개인 로컬 운영에서는 웹 UI와 텔레그램만 사용합니다.

과거 설계 기록:

- [모바일 실기기 E2E 테스트 (보관)](docs/archive/mobile-real-device-e2e.md)
- [Expo 앱 설정 스냅샷](docs/archive/mobile-app.json)
- [한국어 스토어 등록 정보 초안 (보관)](docs/archive/store-listing.ko.json)

WBS 15.8(schemaVersion 2)에서 `devices`·`deviceId`·`pushDelivery*` 레거시 필드는 저장소 읽기 시 자동 마이그레이션 후 제거됩니다. 신규 알림은 텔레그램만 전송합니다. 정책·제거 이력은 [JSON 레거시 필드 정리 계획](docs/json-legacy-fields-deprecation.md) (WBS 14.8·15.8)을 참고하세요.

## 앱 심사·스토어·Railway (보관 — WBS 13.5)

앱스토어/플레이스토어 출시와 Railway 상시 배포는 진행하지 않습니다. 관련 문서·설정·점검 CLI는 제거했고 `docs/archive/`에 보관합니다.

- [앱 심사 준비 체크리스트](docs/archive/app-store-review-prep.md)
- [HTTPS 데모 서버 준비](docs/archive/https-demo-server.md)
- [스토어 스크린샷 제작 가이드](docs/archive/store-screenshots.md)
- [스토어 제출 자산 최종 점검](docs/archive/store-submission-assets.md)
- [개인정보 처리방침 초안](docs/archive/privacy-policy-ko.md)
- [Railway 배포 가이드](docs/archive/railway-deploy.md)
- `docs/archive/railway.json`, `docs/archive/.env.railway.example`

## 테스트 실행

전체 자동 회귀 (`npm test` — **326개**):
...
- [개인용 회귀 테스트 시나리오](docs/full-regression-test-scenarios.md) (WBS 13.10·16.1)
- [개인 PC 로컬 실행 가이드](docs/personal-local-execution-guide.md) (WBS 13.8)
- [개인 PC 백업·복구 정책](docs/personal-backup-policy.md) (WBS 13.7)
- [개인 텔레그램 원격 운영](docs/personal-telegram-operations.md) (WBS 13.9)
- [개인 실사용 백로그](docs/personal-backlog.md) (WBS 14.3)
- [주간 회귀 운영 루틴](docs/personal-weekly-routine.md) (WBS 14.4)
- [KIS/Naver 자동 비교 알림 운영](docs/personal-kis-naver-alert-operations.md) (WBS 14.7)
- [JSON 레거시 필드 정리 계획](docs/json-legacy-fields-deprecation.md) (WBS 14.8)
...
관리자 화면에서는 같은 흐름을 버튼으로 실행할 수 있습니다. `/admin`의 `점검 히스토리` 카드에서 `점검 실행/저장`을 누르면 `/api/observation-history/run`이 읽기 중심 점검과 장중 상태 확인을 실행하고 결과를 `data/observation-history/`에 저장합니다. 검증용 종목을 생성/삭제하는 `--run-state-check`는 웹 버튼 기본값에 포함하지 않습니다.

저장된 히스토리 목록에서는 각 파일의 `상세` 버튼으로 실패/수동 항목의 evidence와 다음 조치를 바로 확인할 수 있습니다. `JSON` 또는 `JSON 다운로드` 버튼은 저장된 점검 스냅샷을 브라우저에서 내려받습니다. 이 상세 API는 `GET /api/observation-history/{fileName}`이며 관리자 토큰 보호 대상입니다.

히스토리 보관 관리는 같은 카드에서 처리합니다. 각 행의 `삭제` 버튼은 해당 `observation-*.json` 파일만 삭제하고, `오래된 기록 정리`는 입력한 보관 개수 기준으로 최근 N개만 남깁니다. 일괄 정리 API는 `POST /api/observation-history/prune`, 개별 삭제 API는 `DELETE /api/observation-history/{fileName}`이며 둘 다 관리자 토큰 보호 대상입니다.

실패/수동 항목은 상세 화면에서 조치 상태(`미처리`, `조치중`, `해결`, `보류`), 조치 메모, 다음 확인일을 저장할 수 있습니다. 조치 메모는 히스토리 원본 파일을 바꾸지 않고 `data/observation-actions.json`에 별도 저장되며, 상세 응답과 JSON 다운로드 스냅샷에 합쳐집니다. 저장 API는 `PATCH /api/observation-history/{fileName}/results/{resultId}/action`이고 관리자 토큰 보호 대상입니다.
...
조회가 실패하면 종목 카드의 실패 사유 패널에 provider별 원인과 수동 입력 안내가 표시됩니다. 우선주나 일부 종목은 공공데이터 행 이름이 보통주와 다르거나 배당 행이 늦게 올라올 수 있으므로, 자동 조회가 막히면 편집에서 `주당 연 배당금`을 직접 입력해 계산을 유지합니다.
...
2026-06-22 방향 전환 기준: 앞으로는 개인 PC 로컬 실행과 텔레그램 확인을 최종 사용 형태로 봅니다. Postgres 전환, 모바일 앱 출시, 스토어 제출, Railway 상시 배포는 새 개발 목표에서 제외하고 정리 후보로 관리합니다. AI 직원 역할·세션 규칙은 [AGENTS.md](AGENTS.md)를 따릅니다.

최근 완료 요약:

- 등록/사용자 UX: 종목 등록 팝업, 매수일 선택 입력, 알림 기준 계산 비교 안내, 자동완성 배지, 기준별 예상 결과 비교, 매수 이유/매도 조건 카드, 추가매수 계산기
- 알림/포트폴리오: 이익금 반납률 알림, 최대 평가이익/반납 금액, 계좌 총 반납률, 매도 판단 대시보드, 종목별 알림 토글, 1시간/오늘 일시정지
...
- 저장소: JsonStore JSON 파일, storageContract, storageFactory, 백업 스냅샷 계약
- 안정화: 서버 연결 실패/캐시 상태 안내, 시세/배당 실패 사유 표시, 종목별 재시도 UX, 시세 품질 배지, 보유/관심/매도 상태 분리, 텔레그램 상태/일시정지/매도/관심/보유 명령, 브라우저 시각 회귀 점검, 로컬 웹앱 실사용 관찰 리포트, 연결 실패 원문 오류 안내형 메시지 전환, 관리자 실사용 이슈 우선순위 표시, 실사용 관찰 체크리스트와 다음 확인 항목 표시, `check:observation` 실사용 smoke check, OBS-003 상태 변경 검증, 사용자 편의 개선 회고, 오늘 확인할 일 카드, 종목 목록 저장 필터, CSV 가져오기/내보내기, 알림 기준 설명 고도화, 배당 API 자동 검증 대시보드, 로컬 장중 실사용 재검증, 로컬 점검 결과 저장/히스토리, 로컬 점검 히스토리 관리자 화면 노출, 관리자 점검 실행/히스토리 저장, 점검 히스토리 상세 보기/다운로드, 점검 히스토리 보관 기간/삭제 관리, 점검 실패 항목 조치 메모
- 모바일/출시 정리 후보: Expo SDK 55 초기 앱, 서버 연결, 익명 기기 저장, 모바일 종목 조회/등록/편집/삭제, 배당 캘린더/알림 기록 상세, Expo Push 토큰 등록과 알림 전송, 실기기 E2E 준비 점검 CLI, 앱 심사 준비 문서, HTTPS 데모 서버 준비, 스토어 메타데이터 초안, 스토어 스크린샷 화면/문구/대체 텍스트 가이드, 스토어 제출 자산 점검 CLI
- 출시 리허설 정리 후보: KIS/공공데이터/텔레그램 외부 API 통합 재점검 CLI와 실행 기록 중 개인 운영에 필요한 실계정 점검만 유지

WBS 14 (개인 로컬 운영·점진 개선)는 14.1~14.8까지 완료했습니다. WBS 15 (개인 운영 안정화와 레거시 정리 2차)는 15.1~15.8까지 완료했습니다. WBS 16 (개인 운영 편의성 개선)은 16.1~16.9까지 완료했습니다. WBS 17 (장중·원격 운영 신뢰도 심화)은 17.1~17.8까지 완료했습니다. WBS 18 (장중 알림·점검 피드백 루프)은 18.1~18.8까지 완료했습니다. WBS 19 (장중 오늘 할 일·알림 연동 심화)는 **19.1~19.8 완료**입니다. WBS 20 (개인 운영 회귀·문서 정합 3차)는 **20.1~20.8 완료**입니다. 이후는 **운영 유지 모드** — 자동 WBS 21+ 없이 백로그 triage만 사용합니다. WBS·백로그·로드맵은 [개발 WBS 및 로드맵](docs/development-roadmap.md)과 [AGENTS.md](AGENTS.md)를 단일 기준으로 봅니다.
