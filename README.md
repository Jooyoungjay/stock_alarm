...
- 가격 비교 이슈 알림 재전송 정책
- 자동 KIS/Naver 비교 이슈 처리 상태와 새 이슈 발생 여부를 반영해 텔레그램 재알림 조건 정리
...
- 증권사 API: 한국투자증권/키움 quote-only adapter 점검 CLI, 주문 기능 차단 가드, KIS 현재가 provider, KIS 토큰 자동 발급/캐시, 실계정 현재가 smoke test CLI, 관리자 KIS 현재가 점검, 종목별 KIS 시장 기준, KIS/Naver 가격 비교와 시장 적용, 가격 차이 이상치 모니터링, 가격 비교 이력 저장, 시장별 가격 비교 추세, 추세 기반 시장 추천, 관심 종목 자동 점검, 자동 점검 결과 알림, 가격 비교 이슈 처리 UX, 환경변수 문서화
