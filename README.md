...
- 보호 대상 운영 API는 `/api/health`, `/api/data-model`, `/api/roadmap`, `/api/observation-issues`, `/api/observation-history`, `/api/observation-history/{fileName}`, `/api/observation-history/run`, `/api/backups`, `/api/check-now`, `/api/dividends/refresh`, `/api/dividend-alerts/check`, `/api/briefing/send`, `/api/telegram/test`입니다.
...
- 관리자 화면에는 서버 상태, 데이터 모델, provider 진단, 백업/복구/삭제, 개발 WBS, 실사용 OBS 이슈, 점검 실행/저장, 점검 히스토리 상세/다운로드를 옮겼습니다.
