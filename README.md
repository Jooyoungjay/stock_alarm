...
- 관리자 KIS/Naver 가격 비교: 같은 종목의 Naver 기준가와 KIS KRX/NXT/통합 가격 차이 확인, 추천 시장 적용, 이상치 기준 초과 시 주의/경고 표시, 최근 비교 이력 저장
...
- KIS/Naver 가격 비교: 같은 종목의 Naver 기준가와 KIS 시장별 가격 차이, 차이율, 실패 사유 확인, 추천 시장 적용, 이상치 기준 초과 여부 확인, 최근 비교 이력 확인
- 배당 provider 진단 확인
- 개발 로드맵과 다음 개발 순서 확인
- 배당 새로고침
- 데이터 백업 생성
...
`/admin`의 `KIS/Naver 가격 비교` 카드는 같은 종목의 Naver 기준가와 KIS KRX/NXT/통합 가격을 나란히 조회합니다. 시장별 가격 차이와 차이율, provider 실패 사유를 볼 수 있고, 가격 차이가 가장 작은 추천 시장을 등록 종목의 KIS 시장 기준으로 바로 적용할 수 있습니다. `이상치 기준 %`를 입력하면 절대 괴리율이 기준 이상인 시장은 주의, 기준의 2배 이상인 시장은 경고로 표시됩니다. 비교를 실행하면 결과와 이상치 판정이 최근 이력으로 저장되어 관리자 화면에서 다시 확인할 수 있습니다.
...
- KIS/Naver 가격 비교 추세 시각화
- 저장된 가격 비교 이력 기반 반복 이상치 표시
...
- 증권사 API: 한국투자증권/키움 quote-only adapter 점검 CLI, 주문 기능 차단 가드, KIS 현재가 provider, KIS 토큰 자동 발급/캐시, 실계정 현재가 smoke test CLI, 관리자 KIS 현재가 점검, 종목별 KIS 시장 기준, KIS/Naver 가격 비교와 시장 적용, 가격 차이 이상치 모니터링, 가격 비교 이력 저장, 환경변수 문서화
