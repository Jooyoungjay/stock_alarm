...
날짜 기준: 2026-06-24 (WBS 14.4·16.4)
...
| # | 항목 | 명령/동작 | 합격 기준 |
|---|---|---|---|
| W-05 | 수동 백업 | 텔레그램 `/backup` 또는 관리자 **백업 생성** | 백업 파일 생성; schema v2면 **레거시 필드 비우기** 체크 권장 (`stripLegacy`) |
...
| W-09 | 배당 진단 샘플 | `/dividend-status` (실패 종목 있으면 1종목 추가) | provider 실패 사유·다음 조치 읽을 수 있음 |
...
| # | 항목 | 합격 기준 |
|---|---|---|
| W-11 | 시세 최신성 | 활성 종목 시세가 **30분** 이내 또는 `/app` **시세 신선도 배너**·실패 사유가 납득 가능 |
...
2. check:observation --live-session (또는 관리자 점검 실행)
3. /status 또는 /check 로 텔레그램 원격 확인
4. 이상 시 personal-backlog에 BL-* 기록
```
...
[ ] W-05 /backup 또는 관리자 백업 (stripLegacy 선택)
