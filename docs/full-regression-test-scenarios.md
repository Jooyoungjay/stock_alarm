...
날짜 기준: 2026-06-22 (WBS 13.10, 테스트 개수 WBS 14.8에서 270개로 갱신)
...
| A-17 | `npm test` | **270개 전부 통과** | 최종 자동 회귀 |
...
| ID | 목적 | 절차 | 합격 기준 |
|---|---|---|---|
| U-01 | 사용자 첫 화면 | `/app` 접속 | `내 계좌 상황`, 포트폴리오 요약, 종목 카드 중심 화면 표시 |
...
| P-03 | 이익금 반납률 | 최고 이익금의 10%, 15% 반납 조건 설정 | 최대 평가이익, 반납 금액, 반납률 표시 |
...
| P-07 | 계좌 요약 | 여러 종목 수량/평단가와 증권사/계좌명 입력 | 계좌 구분/계좌명/통화별 평가금액, 최대 평가이익, 총 반납률 표시 |
...
| 뷰포트 | 페이지 | 확인 항목 |
|---|---|---|
| 1440x900 | `/app` | 내 계좌 상황, 포트폴리오, 등록 팝업, 종목 카드 |
